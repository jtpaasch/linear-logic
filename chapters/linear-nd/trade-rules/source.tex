...
So, we must have a start rule. And indeed, every natural deduction system has a rule that lets us state any initial premise we like, in order to get started. We will call this rule the \startrule/ rule (sometimes it is called the \rulename{axiom} rule).
...
Also, every natural deduction system has a \startrule/ rule of some kind, which gives us a way to start building proof trees. The \rulename{start} rule says that we can assert up front that any particular proposition we are prepared to vouch for is true.
